...
def a1(msg):
	...
	if msg_text[0] == "new_member" and len(msg_text[1]) == 16 and len(msg_text[2]) == 16:
		...
		bot.send_message(msg.from_user.id, "Message should be in format: new_member {Your secret api} {Your API}")
...
def close_orders(order, trade):
	...
	Print("Order is cloced" + trade["ticker"])
